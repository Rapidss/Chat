	...
	def __init__(self):
		...
		while 1:
			...
			if not inc:
				...
				ciphertext = out[0]
				decrypted_msg = self.decrypt(self.key, ciphertext[0], ciphertext[1])
				decrypted_msg = decrypted_msg[:-decrypted_msg[-1]]
				print("{} : {}".format(out[1], str(decrypted_msg.decode()))) # display the message
		return 0
	def send(self):
		while 1:
			msg = input(">> ") # user input
			length = 16 - (len(msg) % 16)#pad input to blocksize
			msg = msg.encode()
			msg += bytes([length])*length
			tup = (self.hwaddr, self.encrypt(msg, self.key)) # construct the tuple
